...
@section('title', 'Configurar Seção Produtos')
@section('page-title', 'Configurar Seção Produtos')
...
@section('content')
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            <div class="mb-4 sm:mb-0">
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-900">
                    Configurar Seção Produtos
                </h2>
                <p class="mt-2 text-sm sm:text-base text-gray-600">
                    Configure a seção de produtos da sua página pública.
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white shadow-lg rounded-xl animate-slide-up">
        <form method="POST" action="{{ route('dashboard.public-page.products.update') }}" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div class="flex items-center space-x-4">
                    <input type="checkbox" 
                           ...
                           name="show_section" 
                           value="1"
                           {{ \App\Models\PublicPageConfig::getConfig('products', 'show', true) ? 'checked' : '' }}
                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                    <label for="show_section" class="text-sm font-medium text-gray-700">
                        Mostrar seção Produtos na página pública
                    </label>
                </div>
                ...
                <div>
                    ...
                    <input type="text" 
                           id="icon" 
                           name="icon" 
                           value="{{ \App\Models\PublicPageConfig::getConfig('products', 'icon', 'fas fa-box') }}"
                    ...
                    <p class="mt-1 text-sm text-gray-500">Use classes do FontAwesome (ex: fas fa-box, fas fa-shopping-cart)</p>
                </div>
                ...
                <div>
                    ...
                    <input type="text" 
                           id="title" 
                           name="title" 
                           value="{{ \App\Models\PublicPageConfig::getConfig('products', 'title', 'Nossos Produtos') }}"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300"
                ...
                <div>
                    ...
                    <textarea id="description" 
                              ...
                              rows="3"
                              class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300"
                              placeholder="Descubra nossa linha completa de produtos de qualidade">{{ \App\Models\PublicPageConfig::getConfig('products', 'description', 'Descubra nossa linha completa de produtos de qualidade') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="number" 
                           id="count" 
                           name="count" 
                           value="{{ \App\Models\PublicPageConfig::getConfig('products', 'count', 6) }}"
                           min="1"
                           max="20"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300"
                           placeholder="6">
                    <p class="mt-1 text-sm text-gray-500">Número de produtos que aparecerão na página pública (1-20)</p>
                </div>
            </div>
