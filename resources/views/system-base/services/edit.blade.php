...
@section('content')
    <div class="bg-white shadow-lg rounded-xl">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Editar Serviço</h3>
            <p class="text-sm text-gray-600">Atualize os dados do serviço</p>
        </div>

        <form action="{{ route('dashboard.services.update', $service) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="price_type" 
                        ...
                        <option value="fixed" {{ old('price_type', $service->price_type ?? 'fixed') == 'fixed' ? 'selected' : '' }}>Preço Fixo</option>
                        <option value="hourly" {{ old('price_type', $service->price_type) == 'hourly' ? 'selected' : '' }}>Por Hora</option>
                        <option value="custom" {{ old('price_type', $service->price_type) == 'custom' ? 'selected' : '' }}>Sob Consulta</option>
                    </select>
                    @error('price_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="category" 
                        ...
                        <option value="consultoria" {{ old('category', $service->category) == 'consultoria' ? 'selected' : '' }}>Consultoria</option>
                        <option value="desenvolvimento" {{ old('category', $service->category) == 'desenvolvimento' ? 'selected' : '' }}>Desenvolvimento</option>
                        ...
                        <option value="suporte" {{ old('category', $service->category) == 'suporte' ? 'selected' : '' }}>Suporte</option>
                    </select>
                    @error('category')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="status" 
                        ...
                        <option value="active" {{ old('status', $service->status) == 'active' ? 'selected' : '' }}>Ativo</option>
                        <option value="inactive" {{ old('status', $service->status) == 'inactive' ? 'selected' : '' }}>Inativo</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            <p class="text-sm text-gray-600">{{ $service->image }}</p>
                ...
                <div class="md:col-span-2">
                    <label for="image" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ $service->image ? 'Nova Imagem do Serviço' : 'Imagem do Serviço' }}
                    </label>
